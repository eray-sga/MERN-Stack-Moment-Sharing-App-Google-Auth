...
router.post("/signup", async (req, res) => {
  try {
    ...
    const accessToken = jwt.sign(
      ...
      { expiresIn: '3m' }
    ); //accessToken 3dk sonra kullanılmaz olacak
...
router.post("/signin", async (req, res) => {
  try {
    ...
    if (!isPasswordCorrect) {
      return res
        .status(404)
        .json({ message: "Giriş bilgilerinizi kontrol edin" });
    } else {
      const accessToken = jwt.sign({
        email: user.email,
        id: user._id
...
router.get('/refresh/:id', async(req, res) => {
  try {
    ...
    jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (err, decodedRefreshToken) => {
      ...
      const accessToken = jwt.sign({email: decodedRefreshToken.email, id: decodedRefreshToken.id}, process.env.ACCESS_TOKEN_SECRET, {expiresIn: '15s'})
